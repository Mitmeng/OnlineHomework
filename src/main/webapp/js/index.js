$(document).ready(function(e) {
			 $('#addquerstions').change(function() {
		            ...
		            switch (index) {
		                ...
		                case "1": //多选
		                    ...
		                    if (index == "2") {
		                        wjdc_list = $(wjdc_list).append('<li class="liclass">  <label> <textarea name="" cols="" rows="" class="input_wenbk btwen_text btwen_text_dx" ></textarea></label> </li>');
		                    }
			...
			/*	$(".btwen_text").val("题目");
				...
				$(".zjxx").live("click", function() {
					//点击增加选项时，先判断现在有多少选项，超过4个，则不能添加
					//题目选项的个数
					var zjxx_html = $(this).prev(".title_itram").children(".kzjxx_iteam").html();
					var op = $(this).parent(".dx_box").children(".title_itram").children(".kzjxx_iteam").length;
					if(op<4){
				...
				$(".swcbj_but").live("click", function() {
					...
					switch(querstionType) {
						...
						case "1": //多选	
							...
							//当前题目的题号
							var number = $(this).parent(".bjqxwc_box").parent(".dx_box").parent(".movie_box").children(".wjdc_list").children("li").children(".tm_btitlt").children(".nmb").text();
							//alert("这是题号"+number);
							//赋值文本框 
							//题目标题
							...
							jcxxxx.children(".title_itram").children(".kzjxx_iteam").each(function() {
								...
								var li = '<li><label class="optionclass" id="myoption"><input name="a'+number+'" type="' + inputType + '" value="'+bjjs_bj+'"><span>' + texte_val_bj + '</span></label></li>';
